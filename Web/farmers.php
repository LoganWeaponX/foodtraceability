...
$conn = mysqli_connect("localhost", "root", "", "food_traceability");
  ...
  $sql = "SELECT f_id, f_name,f_address,f_mobile,f_email,f_username  FROM farmer_reg";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["f_id"]. "</td><td>" . $row["f_name"] . "</td><td>" . $row["f_address"]. "</td><td>" . $row["f_mobile"]. "</td><td>" . $row["f_email"]. "</td><td>" . $row["f_username"]. "</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
